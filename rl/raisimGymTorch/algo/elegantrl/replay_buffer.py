...
class ReplayBuffer:  # for off-policy
    def __init__(self,
        ...
        self.states = np.empty((max_size, num_seqs, state_dim), dtype=np.float32)
        self.actions = np.empty((max_size, num_seqs, action_dim), dtype=np.float32)
        self.rewards = np.empty((max_size, num_seqs), dtype=np.float32)
        self.undones = np.empty((max_size, num_seqs), dtype=np.float32)
    ...
    def update(self, items: Tuple[np.ndarray, ...]):
        self.add_item = items
        ...
        if self.if_use_per:
            data_ids = np.arange(self.p, p, dtype=np.long)
            if p > self.max_size:
                data_ids = np.mod(data_ids, self.max_size)

            for sum_tree in self.sum_trees:
                sum_tree.update_ids(data_ids=data_ids, prob=10.)
    ...
    def sample(self, batch_size: int) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        ids = np.random.randint(sample_len * self.num_seqs, size=(batch_size,))
        ids0 = np.mod(ids, sample_len)
        ids1 = np.floor_divide(ids, sample_len)

        return (torch.tensor(self.states[ids0, ids1],device=self.device),
                torch.tensor(self.actions[ids0, ids1],device=self.device),
                torch.tensor(self.rewards[ids0, ids1],device=self.device),
                torch.tensor(self.undones[ids0, ids1],device=self.device),
                torch.tensor(self.states[(ids0 + 1) % self.cur_size, ids1],device=self.device),)
    ...
    def sample_for_per(self, batch_size: int) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        is_indices = np.hstack(is_indices)
        is_weights = np.hstack(is_weights)

        ids0 = np.mod(is_indices, self.cur_size)
        ids1 = np.floor_divide(is_indices, self.cur_size)
        return (
            torch.tensor(self.states[ids0, ids1],device=self.device),
            torch.tensor(self.actions[ids0, ids1],device=self.device),
            torch.tensor(self.rewards[ids0, ids1],device=self.device),
            torch.tensor(self.undones[ids0, ids1],device=self.device),
            torch.tensor(self.states[(ids0 + 1) % self.cur_size, ids1],device=self.device),
            torch.tensor(is_weights,device=self.device),
            torch.tensor(is_indices,device=self.device),
        )
    # def sample(self, batch_size: int) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
    #     sample_len = self.cur_size - 1
    ...
    #             self.states[(ids0 + 1) % self.cur_size, ids1],)
    ...
    def td_error_update_for_per(self, is_indices: np.ndarray, td_error: np.ndarray):
        prob = np.clip(td_error, 1e-8, 10).pow(self.per_alpha)
        ...
        for env_i in range(self.num_seqs):
            ...
            sum_tree.update_ids(is_indices[slice_i:slice_j], prob[slice_i:slice_j])

    def td_error_update_for_per(self, is_indices_tensor: Tensor, td_error_tensor: Tensor):
        is_indices = is_indices_tensor.cpu().numpy()
        td_error = td_error_tensor.cpu().numpy()
        self.td_error_update_for_per(is_indices, td_error)
        ...
        if if_save:
            for item, name in item_names:
                if self.cur_size == self.p:
                    ...
                    buf_item = np.vstack((item[self.p:self.cur_size], item[0:self.p]))
                file_path = f"{cwd}/replay_buffer_{name}.npy"
                print(f"| buffer.save_or_load_history(): Save {file_path}")
                np.save(file_path, buf_item)

        elif all([os.path.isfile(f"{cwd}/replay_buffer_{name}.npy") for item, name in item_names]):
            max_sizes = []
            for item, name in item_names:
                file_path = f"{cwd}/replay_buffer_{name}.npy"
                print(f"| buffer.save_or_load_history(): Load {file_path}")
                buf_item = np.load(file_path)
...
class SumTree:
    ...
    def __init__(self, buf_len: int):
        ...
        self.tree = np.zeros(self.max_len, dtype=np.float32)
    ...
    def update_ids(self, data_ids, prob=10.):  # 10 is max_prob
        ...
        for depth in range(self.depth - 2):  # propagate the change through tree
            p_ids = np.unique((l_ids - 1) // 2)  # parent indices
    ...
    def important_sampling(self, batch_size: int, beg: int, end: int, per_beta: float) -> Tuple[np.ndarray, np.ndarray]:
        values = (np.arange(batch_size) + np.random.rand(batch_size)) * (self.tree[0] / batch_size)

        leaf_ids, leaf_values = zip(*[self.get_leaf_id_and_value(v) for v in values])
        ...
        assert 0 <= indices.min() and indices.max() < self.buf_len

        prob_ary = leaf_values / self.tree[beg:end].min()
        weights = np.power(prob_ary, -per_beta)
