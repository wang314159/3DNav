...
class Obstacles(object):
    def __init__(self,OrinObsTracks,ContObsTracks_lattice,ContObsTracks_clip,current_frame):
        #在在被控制的车的前面，80m，
        #障碍物的x,y坐标和v,h,t
        self.OrinObsTracks=OrinObsTracks#没有被控制的车，按照数据集数据移动的车，这种车的轨迹可以直接预判,2秒，50帧，
        self.ContObsTracks_lattice=ContObsTracks_lattice#被lattice控制的车，传入轨迹
        self.ContObsTracks_clip=ContObsTracks_clip#被网络控制的车只有五帧那就五帧，
        self.current_frame=current_frame
        self.tracks = []
        self.address_date()


    def update(self,OrinObsTracks,ContObsTracks,current_frame):

        self.OrinObsTracks = OrinObsTracks  # 没有被控制的车，按照数据集数据移动的车，这种车的轨迹可以直接预判,2秒，50帧，
        self.ContObsTracks_lattice = ContObsTracks["lattice"]  # 被lattice控制的车，传入轨迹
        self.ContObsTracks_clip = ContObsTracks["clip"]  # 被网络控制的车只有五帧那就五帧，
        self.current_frame = current_frame
        self.tracks = []
        self.address_date()
    def get_end_conditions(self, ref_line):
        end_s_conditions=[]
        for track in self.tracks:
            s,l=ref_line.velocity_xy2sl_(track["x"],track["y"],track["v"],track["h"])
            if l[0]>4 or l[0]<-4:
                continue

            state=np.array([s[0],s[1],0])
            end_s_conditions.append(dict({"state": state, "time":round(track["t"],2)}))

        return end_s_conditions
    def address_date(self):
        #x,y,v,h,t
        for OrinObsTrack in self.OrinObsTracks:
            index=self.current_frame-OrinObsTrack[FRAME][0]
            for i in range(5):
                for j in range(10):
                    if index+i*10 >len(OrinObsTrack[BBOX])-1:
                        break
                    x=OrinObsTrack[BBOX][index+i*10,0]
                    y=OrinObsTrack[BBOX][index+i*10,1]
                    v=math.sqrt(OrinObsTrack[X_VELOCITY][index+i*10]**2+OrinObsTrack[Y_VELOCITY][index+i*10]**2)
                    dx=OrinObsTrack[BBOX][index+i*10+1,0]-OrinObsTrack[BBOX][index+i*10,0]
                    dy = OrinObsTrack[BBOX][index+i*10 + 1, 1] - OrinObsTrack[BBOX][index+i*10, 1]
                    h=math.atan(dy/dx)
                    t=dict({"x":x,"y":y,"v":v,"h":h,"t":(j+1)*0.2})
                    self.tracks.append(t)

        # for Trajectory in self.ContObsTracks_lattice:
        #     if Trajectory is None:
        #         continue
        #     for i in range(5):
        #         for j in range(10):
        #             if i*10 >len(Trajectory.x)-1:
        #                 break
        #             x=Trajectory.x[i*10]
        #             y=Trajectory.y[i*10]
        #             v=Trajectory.v[i*10]
        #             if i*10>=len(Trajectory.yaw):
        #                 h=Trajectory.yaw[-1]
        #             else:
        #                 h=Trajectory.yaw[i*10]
        #             t = dict({"x": x, "y": y, "v": v, "h": h, "t": (j+1)*0.2})
        #             self.tracks.append(t)

        for  track  in self.ContObsTracks_clip:
            for i in range(5):
                for j in range(10):
                    x=track[i]["x"]
                    y=track[i]["y"]
